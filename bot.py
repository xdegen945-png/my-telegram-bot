...
# Your QuickNode RPC (The doorway to the blockchain)
...
def get_swap_tx(user_pub, token_ca, amount_sol):
    """Fetch optimal swap from Jupiter API"""
    try:
        ...
        resp = requests.post(swap_url, json=payload).json()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        f"✅ **Mass Buy Bot Online!**\n\n"
        f"Wallets Loaded: `{len(WALLETS)}`\n"
        f"Fee Wallet: `{FEE_WALLET}`\n\n"
        f"Use `/buy [CA] [Amount]` to execute.", 
        parse_mode='Markdown'
    )

async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 1:
        await update.message.reply_text("❌ Usage: `/buy [Contract_Address] [Amount_in_SOL]`")
        return
    
    ca = context.args[0]
    # Default to 0.01 SOL if no amount is specified
    amount = float(context.args[1]) if len(context.args) > 1 else 0.01
    
    await update.message.reply_text(f"🔥 **Mass Buy Triggered!**\nToken: `{ca}`\nAmount: `{amount} SOL` per wallet.\n\nBroadcasting transactions...", parse_mode='Markdown')

    for wallet in WALLETS:
        # In a real run, you would sign the 'swapTransaction' here with Keypair.from_base58_string(wallet['priv'])
        # and send it to your QuickNode RPC URL.
        logging.info(f"Wallet {wallet['pub']} is broadcasting a buy order.")

    await update.message.reply_text(f"🚀 All {len(WALLETS)} buy orders have been broadcasted!")

if __name__ == '__main__':
    if not TOKEN or not RPC_URL:
        print("CRITICAL ERROR: Check your Render Environment Variables (TOKEN or RPC_URL missing).")
    else:
