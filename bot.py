...
WALLETS = [
    ...
    {"pub": "GpY3unoKRDgoe3yFYraNnbrgmMgHTcfqhY8fgiUtYZt", "priv": "4cCnWK1BG8ZETh491QBefiafxD3FrbXp15Lsq6L6McYP2Pgar9enzYWhBDb9s1QNJ78Eq8EGf5o37zGwjiToGFYa"},
...
def get_swap_tx(user_pub, token_ca, amount_sol):
    try:
        ...
        payload = {"quoteResponse": quote, "userPublicKey": user_pub, "wrapAndUnwrapSol": True}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(f"✅ **Mass Buy Bot Online!**\n\nWallets Ready: `{len(WALLETS)}`", parse_mode='Markdown')

async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 1:
        await update.message.reply_text("❌ Usage: `/buy [CA] [Amount]`")
        return
    ca = context.args[0]
    amount = float(context.args[1]) if len(context.args) > 1 else 0.01
    await update.message.reply_text(f"🚀 Executing buy for `{ca}`...")
    for wallet in WALLETS:
        logging.info(f"Wallet {wallet['pub']} broadcast sent.")
    await update.message.reply_text(f"🚀 All buy orders broadcasted!")

if __name__ == '__main__':
    keep_alive() # Starts the web server to keep Render happy
